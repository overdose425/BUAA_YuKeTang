...
sessionid = ""  # 需改成自己的
uv_id = "2813" # buaa id
random_stop = True # 看完课程后随机停顿一定时长，避免刷课过快被系统发现
...
def one_video_watcher(video_id, video_name, cid, user_id, classroomid, skuid):
    ...
    url = "https://buaa.yuketang.cn/video-log/heartbeat/"
    get_url = "https://buaa.yuketang.cn/video-log/get_video_watch_progress/?cid=" + \
        str(cid) + "&user_id=" + user_id + "&classroom_id=" + \
        classroomid + "&video_type=video&vtype=rate&video_id=" + \
        str(video_id) + "&snapshot=1&term=latest&uv_id=" + uv_id
    ...
    while val != "1.0" and val != '1':
        ...
        for i in range(50):
            ...
            timestep = min(max_time, timestep+1000*15)
    ...
    if random_stop:
        time.sleep(300 + randint(0, 120))

    return 1


def get_videos_ids(course_name, classroom_id, course_sign):
    get_homework_ids = "https://buaa.yuketang.cn/mooc-api/v1/lms/learn/course/chapter?cid=" + \
        str(classroom_id) + "&term=latest&uv_id=" + \
        uv_id + "&sign=" + course_sign
...
if __name__ == "__main__":
    ...
    user_id_url = "https://buaa.yuketang.cn/edu_admin/check_user_session/"
    ...
    # 然后要获取教室id
    get_classroom_id = "https://buaa.yuketang.cn/mooc-api/v1/lms/user/user-courses/?status=1&page=1&no_page=1&term=latest&uv_id=" + uv_id
    submit_url = "https://buaa.yuketang.cn/mooc-api/v1/lms/exercise/problem_apply/?term=latest&uv_id=" + uv_id
